...
import { BookOpen, ChevronDown, KeyRound, X } from "lucide-react";
        ...
        <div className="p-4 flex flex-col gap-4">
          ...
          <div className="rounded-lg border border-edge overflow-hidden bg-tint/30">
            ...
            <p hidden className="px-3 py-2 text-xs text-muted italic">
              ...
              <code className="font-mono">/public/</code> to show a guide image here.
            </p>
          </div>

          <p className="text-xs text-muted leading-relaxed">
            If you have an Image Chest account, you can link that account to ChainMaker using an API
            key.{" "}
          </p>
          <p className="text-xs text-muted leading-relaxed">
            You can then upload images to Image Chest directly from ChainMaker, allowing for higher
            quality uploads and increased storage space. All uploaded images will be private to your
            account.
            {dbUser?.apiKeyNames.includes("imgChest") && (
              <span className="ml-1 text-accent">A key is currently saved.</span>
            )}
          </p>
          ...
          <div className="flex flex-col gap-1.5">
            ...
            <textarea
              ...
              disabled={loading}
              placeholder={loading ? "Loading…" : "Paste your ImgChest API key here"}
              rows={3}
...
export function UserDropdown() {
  ...
  if (!firebaseUser) {
    return (
      <>
        ...
        {authModalOpen &&
          createPortal(<AuthModal onClose={() => setAuthModalOpen(false)} />, document.body)}
      </>
    );
  }
  ...
  return (
    <div ref={ref} className="relative">
      ...
      {apiKeyModalOpen &&
        createPortal(<ImgChestKeyModal onClose={() => setApiKeyModalOpen(false)} />, document.body)}
    </div>
  );
}
