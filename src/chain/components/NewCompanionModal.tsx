...
export function NewCompanionModal({
  ...
  const [internalImg, setInternalImg] = useState<{ imgId: string; previewUrl: string } | null>(
    null,
  );
  ...
  function calcPickerStyle(): React.CSSProperties {
    ...
    const width = 300;
    const maxHeight = 360;
    const style: React.CSSProperties = { position: "fixed", width, maxHeight, zIndex: 10000 };
  ...
  return (
    <>
      ...
      <div
        ...
        onClick={() => { if (mouseDownOnBackdrop.current) onCancel(); }}
      >
        <div
        ...
        >
          ...
          <div className="shrink-0 flex items-center gap-2 px-4 py-3 border-b border-edge">
            <p className="flex-1 text-sm font-semibold text-ink">New Companion</p>
            <button
              type="button"
              onClick={onCancel}
          ...
          <div className="p-4 flex gap-4">
            ...
            <div className="flex-1 min-w-0 flex flex-col gap-2.5">
              ...
              {imageMode === "external" && (
                <div>
                  <label className="text-xs text-muted font-medium">Image URL</label>
                  <input
                    type="url"
                    value={externalUrl}
              ...
              <div className="grid grid-cols-[5rem_1fr] gap-2">
                <div>
                  <label className="text-xs text-muted font-medium">Age</label>
                  <input
                    type="number"
                    min={0}
                    max={9999}
              ...
              {/* Background summary */}
              <div>
                <label className="text-xs text-muted font-medium">Background</label>
                <input
                  type="text"
              ...
              {/* Personality */}
              <div>
                <label className="text-xs text-muted font-medium">Personality</label>
                <textarea
                  value={personality}
