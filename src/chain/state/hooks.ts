...
export function useUpdateStack() {
  const startUpdate = useCallback(
    (name: string) => useChainStore.getState().startUpdate(name),
    [],
  );
  const finalizeUpdate = useCallback(
    (name: string) => useChainStore.getState().endUpdate(name),
    [],
  );
  return { startUpdate, finalizeUpdate };
}
  ...
  return useCallback(
    (name: string, url: string, insertAfter?: Id<GID.Jump>): Id<GID.Jump> => {
      ...
      setTracked("Add jump", c => {
        ...
        const newJump: Jump = jumpWithDefaults(
          newId,
          name,
          source,
  ...
  return useCallback((jumpId: Id<GID.Jump>) => {
    setTracked("Delete jump", c => {
      ...
      for (const charKey in jump.drawbacks) {
        for (const pId of (jump.drawbacks as any)[charKey] ?? [])
          delete c.purchases.O[pId];
      }

      // Delete scenarios
      for (const charKey in jump.scenarios) {
        for (const pId of (jump.scenarios as any)[charKey] ?? [])
          delete c.purchases.O[pId];
      }
      ...
      for (const charKey in jump.altForms) {
        for (const afId of (jump.altForms as any)[charKey] ?? [])
          delete c.altforms.O[afId];
      }
...
export function useCurrencyExchanges(
  jumpId: Id<GID.Jump>,
  charId: Id<GID.Character>,
) {
  ...
  return { exchanges, addExchange, removeExchange, updateExchange };
}

export function useAddCurrencyExchangeFromDoc(
  jumpId: Id<GID.Jump>,
  charId: Id<GID.Character>,
) {
  return useCallback(
...
export function useRemoveCurrencyExchangeFromDoc(
  jumpId: Id<GID.Jump>,
  charId: Id<GID.Character>,
) {
  return useCallback(
...
export function usePurchase<T extends AbstractPurchase | Drawback>(
  id: Id<GID.Purchase>,
) {
    ...
    (subId: Id<GID.Purchase>) => {
      setTracked("Remove subpurchase", c => {
        ...
        if (parent?.subpurchases)
          removeFromArray(parent.subpurchases.list, subId);
        c.budgetFlag += 1;
      });
    },
    [id],
...
export function useJumpBasicPurchases(
  jumpId: Id<GID.Jump>,
  charId: Id<GID.Character>,
) {
  ...
  return {
    perkIds,
    itemIds,
    actions: { addPurchase, removePurchase, reorderPurchases },
  };
}
  ...
  return useChainStore(
    useShallow(s => {
      ...
      if (
        ...
        return undefined;
      const subtype =
        s.chain?.jumps.O[jumpId]?.purchaseSubtypes.O[purchase.subtype];
...
export function useJumpSectionSubtypeIds(
  jumpId: Id<GID.Jump>,
  ...
  return useChainStore(
    useShallow(s => {
      ...
      if (!subtypes) return [];
      return (
        Object.entries(subtypes.O) as [string, PurchaseSubtype | undefined][]
      )
...
function makeStipendHook(
  field: "companionStipend" | "originStipend",
  label: string,
) {
      ...
      (amount: number) => {
        setTracked(`Change ${label} amount`, c => {
          c.jumps.O[jumpId]![field] = {
            amount,
            currency:
              c.jumps.O[jumpId]![field]?.currency ?? DEFAULT_CURRENCY_ID,
          };
          c.budgetFlag += 1;
        });
      },
...
export const useJumpCompanionStipend = makeStipendHook(
  "companionStipend",
  "companion stipend",
);
export const useJumpOriginStipend = makeStipendHook(
  "originStipend",
  "origin stipend",
);
...
export function useJumpDrawbacks(
  jumpId: Id<GID.Jump>,
  charId: Id<GID.Character>,
) {
  ...
  return {
    drawbackIds,
    actions: { addDrawback, removeDrawback, reorderDrawbacks },
  };
}

/** Returns the scenario IDs for a character's slot in a jump, plus add/remove actions. */
export function useJumpScenarios(
  jumpId: Id<GID.Jump>,
  charId: Id<GID.Character>,
) {
  ...
  return {
    scenarioIds,
    actions: { addScenario, removeScenario, reorderScenarios },
  };
}
...
export function useJumpSettings(
  jumpId: Id<GID.Jump>,
  charId?: Id<GID.Character>,
) {
  return useChainStore(
    useShallow(s => {
      ...
      const isPrimary =
        charId != null
          ? (s.chain?.characters.O[charId]?.primary ?? false)
          : true;
...
export function useJumpNarrative(
  jumpId: Id<GID.Jump>,
  charId: Id<GID.Character>,
) {
...
function queueNotification(
  type: keyof PendingNotifications,
  name: string,
): void {
...
function flushNotifications(): void {
  ...
  const wasWere = (names: string[]) =>
    new Set(names).size === 1 ? "was" : "were";
  const costCosts = (names: string[]) =>
    new Set(names).size === 1 ? "cost" : "costs";
  ...
  if (pending.costUpdated.length > 0)
    toast.info(
      `${formatNames(pending.costUpdated)} ${costCosts(pending.costUpdated)} updated`,
    );
  ...
  if (pending.boosterRemoved.length > 0)
    toast.info(
      `Combo text removed from ${formatNames(pending.boosterRemoved)}`,
    );
}
...
export function useJumpOrigins(
  jumpId: Id<GID.Jump>,
  charId: Id<GID.Character>,
) {
  ...
  const setOrigins = useCallback(
    (
      updater: (
        draft: NonNullable<PartialLookup<LID.OriginCategory, Origin[]>>,
      ) => void,
      ...
      setTracked("Edit origins", c => {
        ...
        if (!jump.origins[charId]) {
          (jump.origins as Record<number, Record<number, Origin[]>>)[
            charId as number
          ] = {};
        }
        c.budgetFlag += 1;
        updater(
          jump.origins[charId] as NonNullable<
...
export function useJumpAltForms(
  jumpId: Id<GID.Jump>,
  charId: Id<GID.Character>,
) {
  ...
  const addAltForm = useCallback((): Id<GID.AltForm> => {
    ...
    setTracked("Add alt-form", c => {
      ...
      const altForm: AltForm = {
        ...
        capabilities: "",
        height: {
          value: Math.round(150 + Math.random() * 40),
  ...
  return {
    altFormIds,
    actions: { addAltForm, removeAltForm, reorderAltForms },
  };
}
  ...
  const unlinkJumpDoc = useCallback(
    (pdfUrl: string | null | undefined) => {
      setTracked("Unlink JumpDoc", c => {
        ...
        for (const key in j.purchases)
          purchaseIds.push(...((j.purchases as any)[key] ?? []));
        for (const key in j.drawbacks)
          purchaseIds.push(...((j.drawbacks as any)[key] ?? []));
        for (const key in j.scenarios)
          purchaseIds.push(...((j.scenarios as any)[key] ?? []));
...
export function useJumpCurrencyConfig(jumpId: Id<GID.Jump>) {
    ...
    (id: Id<LID.Currency>) => {
      setTracked("Remove currency", c => {
        ...
        const fixVal = (val: Value) => {
          for (const sv of val)
            if (sv.currency === id) sv.currency = createId<LID.Currency>(0);
        };
        ...
        for (const cStr in jump.purchases) {
          for (const pId of (jump.purchases as any)[cStr] ?? []) {
            ...
            const bp = p as BasicPurchase;
            if (bp.subpurchases?.stipend)
              fixVal(bp.subpurchases.stipend as Value);
            for (const subId of bp.subpurchases?.list ?? []) {
        ...
        for (const cStr in jump.scenarios) {
          for (const pId of (jump.scenarios as any)[cStr] ?? []) {
            ...
            for (const r of p.rewards) {
              if (
                (r as any).currency !== undefined &&
                (r as any).currency === id
              )
  ...
  return {
    currencies,
    actions: { addCurrency, modifyCurrency, removeCurrency },
  };
}
...
export function useJumpSubtypeConfig(jumpId: Id<GID.Jump>) {
        ...
        for (const cStr in jump.purchases) {
          for (const pId of (jump.purchases as any)[cStr] ?? []) {
            ...
            if (p.subpurchases?.list?.length) {
              ...
              if (targetSt && !targetSt.allowSubpurchases)
                targetSt.allowSubpurchases = true;
            }
          }
        }
  ...
  return {
    subtypes,
    actions: { addSubtype, modifySubtype, removeSubtype, disableSubpurchases },
  };
}
...
export function useJumpOriginCategoryConfig(jumpId: Id<GID.Jump>) {
  ...
  const addOriginCategory = useCallback(() => {
    setTracked("Add origin category", c => {
      ...
      jump.originCategories.O[newId] = {
        name: "",
        singleLine: true,
        multiple: false,
      };
      jump.originCategories.fId = createId<LID.OriginCategory>(
        (newId as number) + 1,
      );
    });
  }, [jumpId]);
...
export function useCharacterRegularPerkIds(
  charId: Id<GID.Character>,
): Id<GID.Purchase>[] {
  return useChainStore(
    useShallow(s => {
...
export function useCharacterRegularItemIds(
  charId: Id<GID.Character>,
): Id<GID.Purchase>[] {
  return useChainStore(
    useShallow(s => {
...
export function useChainPurchaseCategoryConfig(
  ...
  return {
    categories,
    actions: { addCategory, renameCategory, removeCategory },
  };
}
    ...
    if (targetCi < 0 || targetCi >= jumpChunks.length) {
      ...
      for (const jId of srcChunk) {
        ...
        for (const charIdStr in jump.supplementPurchases) {
          const bySup = (jump.supplementPurchases as any)[charIdStr];
          if (!bySup) continue;
          for (const pid of (bySup[suppId as number] ??
            []) as Id<GID.Purchase>[])
            delete c.purchases.O[pid];
...
function migrateSuppChunk(
  ...
  for (const charIdStr in c.characters.O) {
    ...
    if (aggregatedInvestment !== 0) {
      ...
      dst[suppId as number] =
        (dst[suppId as number] ?? 0) + aggregatedInvestment;
    }
  }
}
 ...
 */
export function useJumpZeroChangeWouldShiftData(): {
  enabling: boolean;
  disabling: boolean;
} {
  const chain = useChain();
...
export function useChainSupplementsConfig() {
  ...
  const addCategory = useCallback((suppId: Id<GID.Supplement>) => {
    setTracked("Add supplement category", c => {
      ...
      supp.purchaseCategories.fId = createId<GID.PurchaseCategory>(
        (newCatId as number) + 1,
      );
    });
  }, []);
  ...
  const setInitialJump = useCallback(
    (suppId: Id<GID.Supplement>, newDisplayNumber: number) => {
      const state = useChainStore.getState();
      const supp = state.chain?.supplements.O[suppId];
      if (!supp || !supp.singleJump || supp.initialJump === newDisplayNumber)
        return;
      const jumpChunks = state.calculatedData.jumpChunks;
      const offset = state.chain!.chainSettings.startWithJumpZero ? 0 : 1;
      const oldChunk = jumpChunks?.[supp.initialJump - offset] as
        | Id<GID.Jump>[]
        | undefined;
      ...
      setTracked("Move supplement start", c => {
        ...
        if (strategy === "delete") {
          ...
          shiftSuppContent(
            ...
            jumpChunks,
            suppId,
            newDisplayNumber - oldInitialJump,
          );
        }
        c.budgetFlag += 1;
      });
  ...
  const convertToSingleJump = useCallback(
    (suppId: Id<GID.Supplement>, strategy: "delete" | "shunt") => {
      ...
      const keepChunk = jumpChunks?.[keepChunkIdx] as
        | Id<GID.Jump>[]
        | undefined;
    ...
    (amount: number) => {
      setTracked("Set supplement investment", chain => {
        ...
        if (!jump.supplementInvestments[charId])
          jump.supplementInvestments[charId] = {} as any;
  ...
  const reorderPurchases = useCallback(
    (newIds: Id<GID.Purchase>[]) => {
      setTracked("Reorder purchases", c => {
        ...
        if (!jump) return;
        const all = [
          ...((jump.supplementPurchases[charId]?.[suppId] ??
            []) as Id<GID.Purchase>[]),
        ];
        ...
        (jump.supplementPurchases as any)[charId as number] ??= {};
        (jump.supplementPurchases as any)[charId as number][suppId as number] =
          result;
      });
    },
    [jumpId, charId, suppId, type],
  ...
  useChainStore.setState(
    produce(s => {
      ...
      for (const companionId of companions) {
        ...
        if (!csp[companionId][primaryId])
          csp[companionId][primaryId] = {} as never;
        if (!csp[companionId][primaryId][jId])
          csp[companionId][primaryId][jId] = {} as never;
...
export function useJumpSupplementImports(
  ...
  const removeImport = useCallback(
    (id: Id<GID.Purchase>) => {
      const si = useChainStore.getState().chain?.purchases.O[id] as
        | SupplementImport
        | undefined;
...
export function useSupplementImport(id: Id<GID.Purchase>) {
  ...
  const modify = useCallback(
    (name: string, updater: (p: SupplementImport) => void) => {
      ...
      const after = useChainStore.getState().chain?.purchases.O[id] as
        | SupplementImport
        | undefined;
      ...
      if (oldPct !== newPct)
        applyCSPDelta(primaryId, jId, sId, kept, newPct - oldPct);
    },
    [id],
  );
  ...
  return useCallback((jumpId: Id<GID.Jump>) => {
    ...
    setTracked("Remove duplicate purchases", c => {
      ...
      for (const charKey in j.purchases) {
        (j.purchases as any)[charKey] = dedupe(
          (j.purchases as any)[charKey] ?? [],
        );
      }
      for (const charKey in j.drawbacks) {
        (j.drawbacks as any)[charKey] = dedupe(
          (j.drawbacks as any)[charKey] ?? [],
        );
      }
      for (const charKey in j.scenarios) {
        (j.scenarios as any)[charKey] = dedupe(
          (j.scenarios as any)[charKey] ?? [],
        );
      }
...
export function useSetObsolete() {
  return useCallback(
    (
      purchaseId: Id<GID.Purchase>,
      currentJumpId: Id<GID.Jump>,
      makeObsolete: boolean,
    ) => {
      setTracked(
    ...
    setTracked("Add companion", c => {
      ...
      const character: Character = {
        ...
        personality: draft.personality
          ...
          : {},
        background: {
          summary: draft.backgroundSummary,
          description: draft.backgroundDescription,
        },
        notes: "",
        primary: false,
        originalForm: newAltFormId,
  ...
  return useCallback((charId: Id<GID.Character>) => {
    setTracked("Remove character", c => {
      ...
      for (const jumpId of c.jumpList) {
        ...
        for (const pId of (jump.purchases as any)[charId] ?? [])
          purchasesToDelete.add(pId);
        for (const pId of (jump.drawbacks as any)[charId] ?? [])
          purchasesToDelete.add(pId);
        for (const pId of (jump.scenarios as any)[charId] ?? [])
          purchasesToDelete.add(pId);
        const suppPurchases = (jump.supplementPurchases as any)[charId];
        if (suppPurchases) {
          for (const suppId in suppPurchases) {
            for (const pId of suppPurchases[suppId] ?? [])
              purchasesToDelete.add(pId);
          }
        }

        // Collect alt-forms to delete
        for (const afId of (jump.altForms as any)[charId] ?? [])
          altFormsToDelete.add(afId);

        // Clean up companion-import references in other characters' purchases
        for (const ownerKey in jump.purchases) {
          const list = (jump.purchases as any)[ownerKey] as
            | Id<GID.Purchase>[]
            | undefined;
          if (!list) continue;
...
export function useCompanionImports(
  jumpId: Id<GID.Jump>,
  charId: Id<GID.Character>,
) {
  const importIds = useChainStore(
...
function deepCopyPurchase(
  ...
  const src = snapshot?.[srcId as number] as
    | (typeof c.purchases.O)[typeof srcId]
    | undefined;
  if (!src) return srcId;
  ...
  switch (src.type) {
    ...
    case PurchaseType.Item: {
      ...
      if (!sameJump)
        copy.subtype =
          DefaultSubtype[src.type as PurchaseType.Perk | PurchaseType.Item];
      ...
      if (bp.subpurchases) {
        ...
        for (const subId of bp.subpurchases.list) {
          const newSubId = deepCopyPurchase(
            c,
            subId,
            jumpId,
            charId,
...
function pushPastedPurchase(
  ...
  if (suppId !== undefined) {
    if (!jump.supplementPurchases[charId])
      jump.supplementPurchases[charId] = {};
...
export function useJumpPurchaseGroups(
  ...
  const groupIds = useChainStore(
    useShallow(s => {
      ...
      return (
        Object.entries(registry.O) as [string, PurchaseGroup | undefined][]
      )
        .filter(([, group]) => {
      ...
      setTracked("Reorder group components", c => {
        ...
        if (jumpId == null) {
          ...
          for (let i = 0; i < g.components.length; i++) {
            const p = c.purchases.O[g.components[i]] as
              | BasicPurchase
              | undefined;
            if (p?.jumpId === jumpId) {
...
export function usePastePurchases(
  jumpId: Id<GID.Jump>,
  charId: Id<GID.Character>,
) {
  return useCallback(
...
export function useJumpDocHoverContext(
  ...
  return useMemo(() => {
    ...
    if (!jump) return null;
    return {
      origins: (jump.origins[charId] ?? null) as PartialLookup<
        LID.OriginCategory,
        Origin[]
      > | null,
      originCategories: jump.originCategories as
        ...
        | undefined,
      currencies: jump.currencies as
        | Registry<LID.Currency, Currency>
        | undefined,
    };
  }, [chain, jumpId, charId]);
}
